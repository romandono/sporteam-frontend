...
export class ClubService {
            ...
            .pipe(
              map( resp => {
                const provincias = resp.provincias.map(
                  provincia => new Provincia(provincia.id, provincia.nombre)
                );
                return {
                  provincias
                }
                ...
                .pipe(
                  map( resp => {
                    const clubs = resp.clubs.map( 
                      club => new Club(club.id,club.nombre,club.localidad,club.provincia,club.modalidad,club.image,
  ...
  eliminarClub(club: Club) {
    const url = `${base_url}/club/${club.id}`;
